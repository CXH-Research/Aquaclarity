...
class NILUT(nn.Module):
    def __init__(self, in_features, hidden_features=None, hidden_layers=3, out_features=None, res=True, drop=0.):
        super().__init__()
...
class LatentTransformerBlock(nn.Module):
    def __init__(self, dim, bias):
        ...
        self.naf = nn.Sequential(*[NAFBlock(dim) for _ in range(28)])
    ...
    def forward(self, x):   
        x = self.belta * self.pixel(x) + (1-self.belta)*self.naf(x)
        return x
...
class Model(nn.Module):
    def __init__(self,
        ...
        self.encoder_level1 = nn.Sequential(*[
            TransformerBlock(dim=dim, num_heads=heads[0], bias=bias,window_size=window_size,shift_size=shift_size) for _ in range(num_blocks[0])])

        self.down1_2 = Downsample(dim)  ## From Level 1 to Level 2
        self.encoder_level2 = nn.Sequential(*[
            CATransformerBlock(dim=int(dim * 2 ** 1), num_heads=heads[1], bias=bias) for _ in range(num_blocks[1])])

        self.down2_3 = Downsample(int(dim * 2 ** 1))  ## From Level 2 to Level 3
        self.encoder_level3 = nn.Sequential(*[
            CATransformerBlock(dim=int(dim * 2 ** 2), num_heads=heads[2], bias=bias) for _ in range(num_blocks[2])])

        self.down3_4 = Downsample(int(dim * 2 ** 2))  ## From Level 3 to Level 4
        self.latent = nn.Sequential(*[
            LatentTransformerBlock(dim=int(dim * 2 ** 3), bias=bias) for _ in range(num_blocks[3])])
        ...
        self.decoder_level1 = nn.Sequential(*[
            TransformerBlock(dim=int(dim), num_heads=heads[0], bias=bias,window_size=window_size,shift_size=shift_size) for _ in range(num_blocks[0])])

        self.reduce_chan_ref = nn.Conv2d(int(dim * 2), int(dim), kernel_size=1, bias=bias)
        self.refinement = nn.Sequential(*[
            CATransformerBlock(dim=int(dim), num_heads=heads[0], bias=bias) for _ in range(num_refinement_blocks)])

        self.output = nn.Conv2d(int(dim), out_channels, kernel_size=1,bias=bias)

        lut_size = 64
        self.lut = NILUT(in_features=out_channels, out_features=out_channels, hidden_features=lut_size, hidden_layers=5)
