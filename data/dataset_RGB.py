...
class DataLoaderTrain(Dataset):
    def __init__(self, rgb_dir, target_class='target', img_options=None):
        super(DataLoaderTrain, self).__init__()

        inp_files = sorted(os.listdir(os.path.join(rgb_dir, 'input')))
        tar_files = sorted(os.listdir(os.path.join(rgb_dir, target_class)))

        self.inp_filenames = [os.path.join(rgb_dir, 'input', x) for x in inp_files if is_image_file(x)]
        self.tar_filenames = [os.path.join(rgb_dir, target_class, x) for x in tar_files if is_image_file(x)]
...
class DataLoaderTest(Dataset):
    def __init__(self, inp_dir, target_class='target', img_options=None):
        super(DataLoaderTest, self).__init__()

        inp_files = sorted(os.listdir(os.path.join(inp_dir, 'input')))
        tar_files = sorted(os.listdir(os.path.join(inp_dir, target_class)))

        self.inp_filenames = [os.path.join(inp_dir, 'input', x) for x in inp_files if is_image_file(x)]
        self.tar_filenames = [os.path.join(inp_dir, target_class, x) for x in tar_files if is_image_file(x)]
    ...
    def __getitem__(self, index):
        ...
        tar = Image.open(path_tar).convert('RGB')

        inp = np.array(inp)
        tar = np.array(tar)

        transformed = self.transform(image=inp, target=tar)

        inp = F.to_tensor(transformed['image'])
        tar = F.to_tensor(transformed['target'])
